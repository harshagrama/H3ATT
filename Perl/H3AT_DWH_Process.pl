...
# USAGE:
#   H3AT_DWH_Process
#   H3AT_DWH_Process 1111123423 '01-03-2020 00:00:00' 10 -site 50142884
#
# DESCRIPTION:
...
my $TYPE_USAGE_EVENT    = 11;       # From RT - H3ATT_DWH_PROCESS_TYPE 
my $TYPE_REC_EVENT      = 12;       # From RT - H3ATT_DWH_PROCESS_TYPE
my $TYPE_OTHER_EVENT    = 13;       # From RT - H3ATT_DWH_PROCESS_TYPE (ONE-OFF, ADJUSTMENT, & RECURRING CANCELLATION EVENTS)
...
EOS
    ...
    SELECT NE.NORMALISED_EVENT_ID,
           ...
           C.INVOICE_TXT,
           C.INVOICE_ID,
           C.GENERAL_1 as C_GENERAL_1,
           TRIM(REPLACE(REPLACE(C.GENERAL_2,'[',''),']','')) as C_GENERAL_2, 
           C.GENERAL_3 as C_GENERAL_3, 
...
EOS
    ...
    if ($zType ne $TYPE_EVENT) {
      $zSQLs{EVENTS_NEW} .= " AND NE.EVENT_CLASS_CODE = ? ";
    }
...
EOS
    ...
    #CRM_REF 
    $zSQLs{CRM_REF} = <<"EOS";
    SELECT DISTINCT GENERAL_10 as CRM_REF
...
sub zPrepareCursors
{
    ...
    if (($zType eq $TYPE_USAGE_EVENT) || ($zType eq $TYPE_REC_EVENT) 
        || ($zType eq $TYPE_OTHER_EVENT) || ($zType eq $TYPE_EVENT)) {
        ...
        $opt_debug && &zDebug("Preparing SQL - " . $zSQLs{CHG_REC});
        $zCSRs{CHG_REC} = $zdb->prepare($zSQLs{CHG_REC}) || &zdie(6000, __LINE__, __FILE__, $zdb->errstr);
...
sub zProcessEvent
{
    ...
    foreach my $FileRowRef (@{$arrayRef}) {
        ...
        if ($zType ne $TYPE_EVENT) {
            $zCSRs{EVENTS_NEW}->execute($fileId,$minStartDate,$maxStartDate,$zType) || &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
        }else{
        ...
        while (my $EventRow = $zCSRs{EVENTS_NEW}->fetchrow_hashref("NAME_uc")) {
            ...
            if ( not $processedEvents{$EventRow->{NORMALISED_EVENT_ID}} ) {
                ...
                push @data, $EventRow->{A_PARTY_ID};            #C_PARTY_ID MAPPED TO A_PARTY_ID
                ...
                if ($EventRow->{EVENT_CLASS_CODE} == $EC_RECURRING) {
                    ...
                    my $frequency = $EventRow->{GENERAL_15};
                    ...
                    push @data, $EventRow->{GENERAL_4}/1000;
                    push @data, $EventRow->{GENERAL_5};         # GENERAL_6 -> CHARGE
                    ...
                    if ($frequency =~ /Mon/){
                       push @data, '10';                        # GENERAL_12 -> frequency based on Tariff Name (Monthly)
                    }else{
                       push @data, '20';                        # GENERAL_12 -> frequency based on Tariff Name (Yearly)
                    }
                    my $dscType;
                    if ($frequency =~ /Adv/){
                        $dscType = '1';
                        push @data, $dscType;                   # GENERAL_13 -> is_advance based on Tariff Name (Advance)
                    }else{
                        $dscType = '0';
                        push @data, $dscType;                   # GENERAL_13 -> is_advance based on Tariff Name (Arrears)
                    }
                    
                    push @data, $EventRow->{GENERAL_14};
                    push @data, $dscType;                       # GENERAL_15 -> discount_type
                    push @data, $EventRow->{GENERAL_5};         # GENERAL_16 -> discount Amount on product
                    ...
                    while (my $crmRow = $zCSRs{CRM_REF}->fetchrow_hashref("NAME_uc")) {
                         ...
                         my @chgData;
                         push @chgData, $EventRow->{CHARGE_ID};
                         push @chgData, $EventRow->{C_LAST_MODIFIED};
                         push @chgData, $EventRow->{CHARGE};
                         push @chgData, $EventRow->{ACCOUNT_ID};
                         push @chgData, $EventRow->{UNINVOICED_IND_CODE};
                         push @chgData, $EventRow->{NORMALISED_EVENT_ID};
                         push @chgData, $EventRow->{SERVICE_ID};
                         push @chgData, $EventRow->{CUSTOMER_NODE_ID};
                         push @chgData, $EventRow->{CHARGE_DATE}; 
                         push @chgData, $EventRow->{TARIFF_ID};
                         push @chgData, $EventRow->{INVOICE_TXT};
                         push @chgData, $EventRow->{INVOICE_ID};
                         push @chgData, $EventRow->{C_GENERAL_1};
                         push @chgData, $EventRow->{C_GENERAL_2};
                         push @chgData, $EventRow->{C_GENERAL_3};
                         push @chgData, $EventRow->{C_GENERAL_4};
                         push @chgData, $EventRow->{C_GENERAL_5};
                         push @chgData, $EventRow->{C_GENERAL_6};
                         push @chgData, $EventRow->{C_GENERAL_7};
                         push @chgData, $EventRow->{C_GENERAL_8};
                         push @chgData, $EventRow->{GENERAL_11};       #GENERAL_9 -> NE.GENERAL_11 (CO:FI)
                         push @chgData, $EventRow->{C_GENERAL_10};
                         
                         #print "NORMALISED_EVENT_ID = ".$EventRow->{NORMALISED_EVENT_ID}."\n";
                         if ($EventRow->{C_GENERAL_2} eq 'DSC'){
                            #print "DSC: NORMALISED_EVENT_ID = ".$EventRow->{NORMALISED_EVENT_ID}."\n";
                            
                            @{$recCharge{$EventRow->{SERVICE_ID}}{$crmRow->{CRM_REF}}{$EventRow->{PERIOD_START_DATE}}{NE}{DSC}}=@data; 
                            @{$recCharge{$EventRow->{SERVICE_ID}}{$crmRow->{CRM_REF}}{$EventRow->{PERIOD_START_DATE}}{CHG}{DSC}}=@chgData; 
                            #print "@data \n";
                         }else{
                            #print "REC: NORMALISED_EVENT_ID = ".$EventRow->{NORMALISED_EVENT_ID}."\n";
                            @{$recCharge{$EventRow->{SERVICE_ID}}{$crmRow->{CRM_REF}}{$EventRow->{PERIOD_START_DATE}}{NE}{REC}}=@data;
                            @{$recCharge{$EventRow->{SERVICE_ID}}{$crmRow->{CRM_REF}}{$EventRow->{PERIOD_START_DATE}}{CHG}{REC}}=@chgData; 
                            #print "@data \n";                            
                         }
                    }
                    ...
                    push @data, $EventRow->{EVENT_TYPE_CODE} - 10000;
                    push @data, $EventRow->{EVENT_SUB_TYPE_CODE} - 200000;
                    push @data, $EventRow->{DURATION};
                    push @data, $EventRow->{CHARGE};
                    
                    push @data, $EventRow->{C_GENERAL_2};                   # RATE_BAND -> CHARGE.GENERAL_2
                    ...
                    if ($splitStr[2] ne '') {
                        ...
                        push @data, '';                                     # GENERAL_4 -> RATEPLAN
                    }
                    push @data, $EventRow->{GENERAL_20};                    # GENERAL_5 -> GENERAL_20
                    push @data, $EventRow->{A_PARTY_NAME};                  # GENERAL_6 -> A_PARTY_NAME (VAS Caller)
                    push @data, $EventRow->{GENERAL_7};
                    push @data, $EventRow->{GENERAL_8};
                    push @data, $EventRow->{GENERAL_9};
                    push @data, $EventRow->{GENERAL_10};
                    ...
                    $opt_testing || $zCSRs{I_NE}->execute(@data) || &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
                    $zStats{INSERT_CNT}++;
        ...
        }# End of while loop for each event
        
        if ($event_found) {
            $opt_testing || $zdb->commit()|| &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
        }
        
    }#end of For loop for each event file
    

    #Process Recurring Charge/Recurring Discount     
    foreach my $serId (keys %recCharge) {
        foreach my $crmRef (keys %{$recCharge{$serId}}) {  
            foreach my $nePsDate (keys %{$recCharge{$serId}{$crmRef}}) {
                my @neRcData;
                my @chgRcData;
                my @neRcdData;
                my @chgRcdData;
                ...
                if (defined $recCharge{$serId}{$crmRef}{$nePsDate}{NE}{REC}) {
                    @neRcData  = @{$recCharge{$serId}{$crmRef}{$nePsDate}{NE}{REC}};
                    @chgRcData = @{$recCharge{$serId}{$crmRef}{$nePsDate}{CHG}{REC}};
                    #print "**** CRM_REF = ".$crmRef." RC PERIOD_START_DATE = ".$nePsDate." NE_ID =".$recCharge{$serId}{$crmRef}{$nePsDate}{NE}{REC}[0]."\n";
                    $neIdRc = $recCharge{$serId}{$crmRef}{$nePsDate}{NE}{REC}[0];
                    
                     if(defined $recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}){
                        print "RECURRIND DISCOUNT: ".$recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}[24]."\n";
                        $neRcData[34] = $recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}[24];            #NE.GELERAL_16 Discount amount on the product
                     }
                    
                    $opt_v && zDebug("inserting entry into table - H3ATT_DWH_NORMALISED_EVENT");
                    $opt_testing || $zCSRs{I_NE}->execute(@neRcData) || &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
                    $zStats{INSERT_CNT}++;
                    
                    # Insert Recurring Charge row into the charge table.
                    $opt_v && zDebug("inserting entry into table - H3ATT_DWH_CHARGE");
                    $opt_testing || $zCSRs{I_CHARGE}->execute(@chgRcData) || &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
                    $zStats{INSERT_CNT}++;
                }
                if(defined $recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}){
                    @neRcdData  = @{$recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}};
                    @chgRcdData = @{$recCharge{$serId}{$crmRef}{$nePsDate}{CHG}{DSC}};
                    $chgRcdData[5] = $neIdRc;           #MAP REC NORMALISED_EVENT_ID TO DSC CHARGE RECORD                  
                    #print "**** CRM_REF = ".$crmRef." RCD PERIOD_START_DATE = ".$nePsDate." NE_ID =".$recCharge{$serId}{$crmRef}{$nePsDate}{NE}{DSC}[0]."\n";
                                       
                    # Insert Recurring Charge row into the charge table. 
                    $opt_v && zDebug("inserting entry into table - H3ATT_DWH_CHARGE");
                    $opt_testing || $zCSRs{I_CHARGE}->execute(@chgRcdData) || &zdie(6001, __LINE__, __FILE__, $zdb->errstr);
                    $zStats{INSERT_CNT}++;
...
$MODULE_NAME task_id effective_start_date process_type [-testing] [-debug] [-v]
    ...
    process_type            Process type code from RT - H3ATT_DWH_PROCESS_TYPE
                            ...
                            11 - Usgae Event
                            12 - Recurring Event
                            13 - Other Events (Recurring Cancellation, Adjustment & One-Off Event)
                            50 - Product
